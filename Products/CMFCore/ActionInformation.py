...
class ActionInformation(SimpleItem):
    ...
    def edit(self,
        ...
        if condition is not _unchanged:
            if condition and isinstance(condition, string_types):
        ...
        if action is not _unchanged:
            if action and isinstance(action, string_types):
                action = Expression(action)
            self.setActionExpression(action)
        if icon_expr is not _unchanged:
            if icon_expr and isinstance(icon_expr, string_types):
    ...
    def getActionExpression(self):
        ...
        if expr and isinstance(expr, string_types):
